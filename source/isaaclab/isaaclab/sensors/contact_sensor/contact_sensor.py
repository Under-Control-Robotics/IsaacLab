...
class ContactSensor(SensorBase):
    ...
    def _initialize_impl(self):
        ...
        self._contact_physx_view = self._physics_sim_view.create_rigid_contact_view(
            ...
            max_contact_data_count=self.cfg.max_contact_data_count_per_prim
            * len(body_names)
            * self._num_envs,
        )
        ...
        if self.cfg.track_contact_points:
            ...
            self._contact_position_aggregate_buffer = torch.full(
                (
                    self._num_bodies * self._num_envs,
                    self.contact_physx_view.filter_count,
                    3,
                ),
                torch.nan,
        ...
        if len(self.cfg.filter_prim_paths_expr) != 0:
            ...
            if self.cfg.history_length > 0:
                ...
                self._data.force_matrix_w_history = self._data.force_matrix_w.unsqueeze(
                    1
                )
    ...
    def _update_buffers_impl(self, env_ids: Sequence[int]):
        ...
        if len(self.cfg.filter_prim_paths_expr) != 0:
            ...
            if self.cfg.history_length > 0:
                self._data.force_matrix_w_history[env_ids] = (
                    self._data.force_matrix_w_history[env_ids].roll(1, dims=1)
                )
                self._data.force_matrix_w_history[env_ids, 0] = (
                    self._data.force_matrix_w[env_ids]
                )
        ...
        if self.cfg.track_contact_points:
            ...
            agg = torch.full(
                ...
                float("nan"),
                device=self._device,
                dtype=buffer_contact_points.dtype,
            )
            if total > 0:
                ...
                deltas = torch.arange(
                    total, device=counts.device
                ) - block_starts.repeat_interleave(counts)
                flat_idx = starts[row_ids] + deltas

                pts = buffer_contact_points.index_select(0, flat_idx)
                agg = agg.zero_().index_add_(0, row_ids, pts) / counts.clamp_min(
                    1
                ).unsqueeze(1)
                agg[counts == 0] = float("nan")

            self._contact_position_aggregate_buffer[:] = agg.view(
                self._num_envs * self.num_bodies, -1, 3
            )
            self._data.contact_pos_w[env_ids] = (
                self._contact_position_aggregate_buffer.view(
                    self._num_envs,
                    self._num_bodies,
                    self.contact_physx_view.filter_count,
                    3,
        ...
        if self.cfg.track_air_time:
            ...
            is_first_contact = (
                self._data.current_air_time[env_ids] > time_threshold
            ) * is_contact
            is_first_detached = (
                self._data.current_contact_time[env_ids] > time_threshold
            ) * ~is_contact
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        ...
        if debug_vis:
            ...
            if not hasattr(self, "contact_visualizer"):
                ...
                # Force vector.
                self.contact_force_visualizer = VisualizationMarkers(
                    self.cfg.force_visualizer_cfg
                )
    ...
    def _resolve_force_to_arrow(self, force: torch.Tensor):
        # arrow-scale
        arrow_scale = torch.tensor([0.05], device=self.device).repeat(
